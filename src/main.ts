...
function handleUninstall() {
	try {
		...
		const isModified = (curr: fs.Stats) =>
			new Date(curr.mtime) >= new Date(curr.atime);
			...
			if (isFileExists(curr, prev) && isModified(curr)) {
				fs.readFile(uninstalledPath, async (err, uninstalled) => {
					try {
						...
						const extensionName = `tabnine-plus-${extension.packageJSON.version}`;
